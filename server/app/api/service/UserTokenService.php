...
class UserTokenService
{
    ...
    public static function setTokenById($user_id, int $terminal)
    {
        $time = time();
        $userSession = UserSession::where([['user_id', '=', $user_id], ['terminal', '=', $terminal]])->find();
        ...
        if ($userSession) {
            ...
            $userTokenCache->deleteUserInfo($userSession->token);
            //重新获取token
            $userSession->token = create_token($user_id);
            $userSession->expire_time = $expireTime;
            $userSession->update_time = $time;
            $userSession->save();
        } else {
