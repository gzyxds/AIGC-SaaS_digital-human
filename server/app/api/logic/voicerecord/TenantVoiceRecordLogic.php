...
namespace app\api\logic\voicerecord;
...
class TenantVoiceRecordLogic extends BaseLogic
{
     ...
     */
    public function queryUrlById($voiceId, $uid)
    {
        $info = TenantVoiceRecord::query()->with('file')->where(['id' => $voiceId, 'uid' => $uid])->findOrEmpty()->toArray();
        return isset($info['file']['uri']) ? FileService::getFileUrl($info['file']['uri']) : null;
