...
use app\common\{enum\YesNoEnum,
    model\avatar\TenantAiAvatarRecord,
    model\video\TenantVideo,
    model\voice\TenantVoice,
    model\voicerecord\TenantVoiceRecord
};
...
class UserLogic extends BaseLogic
{
    ...
    public static function center(array $userInfo): array
    {
        ...
        if (in_array($userInfo['terminal'], [UserTerminalEnum::WECHAT_MMP, UserTerminalEnum::WECHAT_OA, UserTerminalEnum::PC, UserTerminalEnum::H5])) {
        ...
        $imageNum= TenantVideo::query()->where(['uid' => $userInfo['user_id']])->count();
        $avatarNum= TenantAiAvatarRecord::query()->where(['uid' => $userInfo['user_id']])->count();
        $timbreNum = TenantVoiceRecord::query()->where(['uid' => $userInfo['user_id']])->count();
        $voiceNum = TenantVoice::query()->where(['uid' => $userInfo['user_id']])->count();
        // 数字形象数量
        $user['image_num'] = $imageNum;
        // 数字分身数量
        $user['avatar_num'] = $avatarNum;
        // 定制音色数量
        $user['timbre_num'] = $timbreNum;
        // 克隆声音数量
        $user['voice_num'] = $voiceNum;
        // 统计信息
        $user['static'] = [
            [
                'label' => "数字形象",
    ...
    public static function info(int $userId)
    {
        $user = User::where(['id' => $userId])
            ->field('id,sn,sex,account,password,nickname,real_name,avatar,mobile,create_time,user_money,total_recharge_amount')
            ->findOrEmpty();
        $user['has_password'] = !empty($user['password']);
     ...
     */
    public static function setUserInfo(int $userId, array $params)
    {
        try {
            if (isset($params['avatar'])) {
                $params['avatar'] = FileService::setFileUrl($params['avatar']);
            }
            $data = [
                'avatar'   => $params['avatar'],
                'nickname' => $params['nickname'],
            ];
            return User::update($data, ['id' => $userId]);
        } catch (\Exception $e) {
            self::$error = $e->getMessage();
            return false;
        }
     ...
     */
    public static function setPassword(array $params, int $userId)
    {
        try {
            $user = User::query()->findOrEmpty($userId);
            if ($user->isEmpty()) {
                throw new \Exception('用户不存在');
            }
