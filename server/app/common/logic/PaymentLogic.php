...
class PaymentLogic extends BaseLogic
{
    ...
    public static function getPayWay($userId, $terminal, $params)
    {
        try {
            ...
            foreach ($pay_way as $k => &$item) {
                ...
                // 充值时去除余额支付
                if (($params['from'] == 'recharge' || $params['from'] == 'pc_recharge') && $item['pay_way'] == PayEnum::BALANCE_PAY) {
                    unset($pay_way[$k]);
                }
            }
     ...
     * @return void
     * @author yfdong
     * @date 2024/11/11 00:07
     */
    public static function getPowerPayWay($terminal): array
    {
        $pay_way = TenantPayWay::alias('pw')
            ->join('tenant_pay_config dp', 'pw.pay_config_id = dp.id')
            ->where(['pw.scene' => $terminal, 'pw.status' => YesNoEnum::YES])
            ->field('dp.id,dp.name,dp.pay_way,dp.icon,dp.sort,dp.remark,pw.is_default,pw.scene')
            ->order('pw.is_default desc,dp.sort desc,id asc')
            ->select()
            ->toArray();
        foreach ($pay_way as $k => &$item) {
