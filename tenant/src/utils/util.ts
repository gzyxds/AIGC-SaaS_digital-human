...
export const formatSecondsToMinutes = (seconds: number): string => {
    ...
    const minutePart = minutes > 0 ? `${minutes}分` : '' // 如果分钟为 0，则不显示
    const secondPart =
        remainingSeconds > 0 ? `${remainingSeconds.toString().padStart(2, '0')}秒` : '' // 如果秒数为 0，则不显示

    return `${minutePart}${secondPart}` || '0秒' // 如果没有分钟和秒数，显示 `0秒`
}
