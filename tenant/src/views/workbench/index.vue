            ...
            <el-card class="!border-none mb-4 flex-1" shadow="never">
                ...
                <div class="flex flex-wrap">
                    <div class="w-1/2 md:w-1/4">
                        <div class="leading-10">今日销售额</div>
                    ...
                    <div class="w-1/2 md:w-1/4">
                        <div class="leading-10">新增作品数</div>
                        <div class="text-6xl">{{ workbenchData.today.today_works }}</div>
                        <div class="text-tx-secondary text-xs">
                            总：{{ workbenchData.today.total_works }}
                        </div>
                    </div>
                </div>
            </el-card>
            ...
            <el-card class="!border-none mb-4 lg:mb-0 w-full lg:w-2/3" shadow="never">
                ...
                <div>
                    <v-charts
                        ref="worksChart"
                        style="height: 350px"
                        :option="workbenchData.worksOption"
                        :autoresize="true"
                    />
                </div>
            </el-card>
...
const worksChart = useComponentRef(vCharts)
...
const getData = () => {
    getWorkbench()
        .then((res: any) => {
            ...
            workbenchData.works = res.works
            workbenchData.support = res.support

            // 清空echarts 数据
            workbenchData.worksOption.xAxis.data = []
            workbenchData.worksOption.series[0].data = []
            workbenchData.saleOption.xAxis.data = []
            workbenchData.saleOption.series[0].data = []

            // 写入从后台拿来的数据
            res.works.date.reverse().forEach((item: any) => {
                workbenchData.worksOption.xAxis.data.push(item)
            })
            res.works.list[0].data.reverse().forEach((item: any) => {
                workbenchData.worksOption.series[0].data.push(item)
            })
            res.sale.date.reverse().forEach((item: any) => {
                workbenchData.saleOption.xAxis.data.push(item)
            })
            res.sale.list[0].data.reverse().forEach((item: any) => {
                if (item <= 100) {
                    item = {
                        value: item,
                        itemStyle: {
...
const updateColor = () => {
    workbenchData.worksOption.series[0].color = settingStore.theme
    workbenchData.worksOption.series[0].lineStyle.color = settingStore.theme
    workbenchData.worksOption.series[0].areaStyle.color.colorStops = [
        {
            offset: 0,
            color: settingStore.theme
        },
    ...
    worksChart.value?.clear()
    saleChart.value?.setOption(workbenchData.saleOption)
    worksChart.value?.setOption(workbenchData.worksOption)
}
